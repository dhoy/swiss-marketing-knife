...
from PyQt5.QtWidgets import QApplication, QMainWindow, QMessageBox, QCompleter, qApp, QWidget,\
    QLabel, QSizePolicy, QVBoxLayout, QTableWidgetItem, QFileDialog, QDialog,\
    QInputDialog
...
class FTPWindow(QMainWindow, Ui_MainWindow):
    ...
    def choose_file(self):
        ...
        filename, wtf = QFileDialog.getOpenFileName(self, 'Choose file.', dir_path, "Flat Files (*.csv)")
    ...
    def connect_ftp(self, company='ExactTarget'):
        try:
            if company == 'ExactTarget':
                ...
                host = 'sftp.crosscountrycomputer.com'
                #user = 'ccftpExt'
                user = 'ccftpEsm'
                password = 'STia6#Rx'
                #old password
                #password = 'ExT^23'
                #new password 08-21-2016
                #password = 'TuWpR9$V'
                #password = 'TMp32Qk!'
...
#             self.import_csv.import_finished.connect(self.import_finished)
    ...
    def on_progress(self, pro):
        ...
        elif self.import_type == "btn_dl_cc":
...
class UploadCrossCountry(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            serverLocation = os.path.join(r'\\esmserver\inkpixi\ONLINE_MARKETING\Salesforce Marketing Cloud\EXPORTS\from Exact Target\PIXI_DMCID_Subs', upload_file)
        ...
        with open(self.cross_file, 'r') as f:
            ...
            for dt in reader:
                if dt[2]:
                    ...
                    lsDt.append(datetime.strptime(dt[2], '%m/%d/%Y'))
            f.close()
        maxDt = max(lsDt)
...
class GetTopFifty(QThread):
    ...
    def run(self):
        ...
        cur.execute("""SELECT TOP(50) 
                                LEFT(od.SKU, 4) SKU,
                                s.designName design, 
                                COUNT(DISTINCT o.OrderNumber) orders,
                                CAST(CAST(SUM(ISNULL(od.QuantityOrdered, 0) - ISNULL(od.QuantityReturned, 0)) AS DECIMAL(10,2)) AS INT) pieces,
                                CAST(SUM((ISNULL(od.QuantityOrdered, 0) - ISNULL(od.QuantityReturned, 0)) * od.PricePerUnit) AS DECIMAL(10,2)) sales
                        FROM    
                            dbo.Orders o
                        JOIN    
                            dbo.[Order Details] od ON o.OrderNumber = od.OrderNumber
                        JOIN    
                            Reporting.report.tblSku s ON s.rootSKU = LEFT(od.sku,4)
                        WHERE    
                            o.OrderDate BETWEEN  '""" + self.from_dt + """' AND '""" + self.to_dt + """'
                        AND    
                            SUBSTRING(SKU, 6, 3) = '""" + self.sku + """' 
                        GROUP BY 
                            LEFT(od.SKU, 4), s.designName
                        ORDER BY 
                            CAST(SUM((ISNULL(od.QuantityOrdered, 0) - ISNULL(od.QuantityReturned, 0)) * od.PricePerUnit) AS DECIMAL(10,2)) DESC""")
        ds = cur.fetchall()
